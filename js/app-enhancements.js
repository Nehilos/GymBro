...
async function ensureAllDriveDatabasesV7(initial=false){if(!driveFolders?.databaseFolderId)return[];const files=await listDatabaseFiles(driveFolders.databaseFolderId),have=new Set(files.map(x=>x.name)),missing=THALYS_REQUIRED_DB_V7.filter(x=>!have.has(x));if(!missing.length)return[];const p=databasePayloads(),made=[];for(const name of missing){try{let data=p[name];if(name.startsWith('lang_'))data=await (await fetch(`./${name}?v=22`,{cache:'no-store'})).json();if(data===undefined)data=[];await uploadDriveFile(name,JSON.stringify(data),'application/json',driveFolders.databaseFolderId,true);made.push(name)}catch(e){console.warn(name,e)}}return made;}
...
loadPhotosFromDriveFolder=async function(){
  ...
  const q=`'${folder.id}' in parents and trashed=false and mimeType contains 'image/'`;
  const resp=await gapi.client.drive.files.list({q,fields:'files(id,name,mimeType,createdTime,modifiedTime)',orderBy:'createdTime desc',pageSize:100});
  ...
  for(const f of files){
    ...
    try{
      const rr=await fetch(`https://www.googleapis.com/drive/v3/files/${f.id}?alt=media`,{headers:{Authorization:`Bearer ${getAccessToken()}`}});
      if(!rr.ok)continue;
      const blob=await rr.blob(),base64=await blobToDataURL(blob);
