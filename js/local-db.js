(function () {
  ...
  async function warmOfflineAppShell() {
    ...
    try{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('thalys-manual-offline-')&&k!=='thalys-manual-offline-v0.23').map(k=>caches.delete(k)));}catch(_){}
    const cache = await caches.open('thalys-manual-offline-v0.23');
    let saved = 0;
